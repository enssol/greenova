...
class TablesModule {
  ...
  exportToCSV(table, filename) {
    ...
    const headers = Array.from(rows[0].querySelectorAll('th')).map(cell => { var _a; return `"${((_a = cell.textContent) === null || _a === void 0 ? void 0 : _a.trim().replace(/"/g, '""')) || ''}"`; });
    // Extract data rows
    const data = rows.slice(1).map(row => {
      return Array.from(row.querySelectorAll('td')).map(cell => { var _a; return `"${((_a = cell.textContent) === null || _a === void 0 ? void 0 : _a.trim().replace(/"/g, '""')) || ''}"`; });
      });
  ...
  init() {
    ...
    if (typeof window.PathDeps !== 'undefined') {
      document.body.addEventListener('htmx:afterRequest', (e) => {
        ...
        if (!htmxEvent.detail.successful || htmxEvent.detail.xhr.method === 'GET')
        )
          return;
...
class FormsModule {
  ...
  handleFilterFormSubmit(form) {
    ...
    if (filterCount) {
      const activeFilters = Array.from(form.querySelectorAll('select, input[type="text"]')).filter(el => {
        const element = el;
  ...
  init() {
    ...
    document.addEventListener('click', (e) => {
      ...
      if (target.matches('.obligation-link') || target.closest('.obligation-link')) {
        ...
        const projectId = (_a = document.querySelector('input[name="project_id"]')) === null || _a === void 0 ? void 0 : _a.value;
        if (projectId) {
          sessionStorage.setItem('lastProjectId', projectId);
        }
      }
